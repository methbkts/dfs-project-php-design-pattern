<section id="section" class="projects-section bg-light">
    <div class="container">
        <form class="form-signin">
            <img class="" src="/public/img/logo_transparent.png" alt="" width="30%" height="30%">
            <h1 class="h3 mb-3 font-weight-normal">Connectez-vous</h1>
            <label for="inputEmail" class="sr-only">Email ou pseudo</label>
            <input type="email" id="inputEmail" class="form-control" placeholder="Email ou pseudo" required autofocus>
            <label for="inputPassword" class="sr-only">Password</label>
            <input type="password" id="inputPassword" class="form-control" placeholder="Mot de passe" required>
            <div class="checkbox mb-3">
                <label>
                    <input type="checkbox" value="remember-me"> Se souvenir de moi
                </label>
            </div>
            <button class="btn btn-lg btn-primary btn-block shadow-4dp" type="submit">Connexion</button>
        </form>
    </div>
</section>